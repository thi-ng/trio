...
*** Alt implementation
#+BEGIN_SRC clojure :noweb-ref memstore
  (defrecord PlainMemoryStore2 [spo pos ops size]
    PStatementAccess
    (add-statement
      [_ [s p o :as t]]
      (if (-> spo (get s nil) (get p nil) (get t nil))
        ...
        (let [s (get (find spo s) 0 s)
          ...
          (PlainMemoryStore2.
           ...
           (update-in ops [o p] d/set-conj t)
           (inc size)))))
    (add-bulk [_ statements]
      (loop [changed? false, spo spo, pos pos, ops ops, size size, xs statements]
        (if xs
          (let [[s p o :as t] (first xs)]
            (if (-> spo (get s nil) (get p nil) (get t nil))
              (recur changed? spo pos ops size (next xs))
              (let [s (get (find spo s) 0 s)
                    p (get (find pos p) 0 p)
                    o (get (find ops o) 0 o)
                    t (Triple. s p o nil)]
                (recur
                 ...
                 (next xs)))))
          (if changed?
            (PlainMemoryStore2. spo pos ops size)
            _))))
    ...
    (select
      ...
      (if s
        (if p
          ...
          ;; s nil o / s nil nil
          (if o
            (->> (spo s) vals (apply concat) (filter #(= o (% 2))))
        ...
        (if p
          ...
          (if o
            ;; nil nil o
            (->> (ops o) vals (apply concat))
            ;; nil nil nil
            (mapcat #(->> % vals (mapcat identity)) (vals spo)))))))
#+END_SRC
