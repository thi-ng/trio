...
Some use cases require support for aliasing resources stored in
triples. For example in RDF resources can be declared as equal via an
=owl:sameAs= relationship.

The =AliasStore= defined below achieves this goal by wrapping an
...
chosen. In the case of the =AliasStore= this means any triples
...
ones. Aliases can be defined and removed at any time.

#+BEGIN_SRC clojure :noweb-ref alias-store
  (defn rewrite-alias*
    [ds pred id p q]
    (if (pred ds p)
      (let [xs (apply select ds (assoc [nil nil nil] id p))
            ds (remove-bulk ds xs)]
        (add-bulk ds (map #(assoc % id q) xs)))
      ds))
  ...
  (defrecord AliasStore [store aliases]
    PStatementAccess
    (add-statement
      [_ [s p o]]
      (let [t [(or (u/canonical aliases s) s)
        ...
        (AliasStore. (add-statement store t) aliases)))
    (add-bulk
      [_ statements]
    ...
    (register [_ p] (AliasStore. store (u/register aliases p)))
    (unregister
      [_ p]
      (if (u/canonical? aliases p)
        (let [q (first (disj (u/component aliases p) p))
          ...
          (AliasStore. store aliases))
        (AliasStore. store (u/unregister aliases p))))
    (unified? [_ p q] (u/unified? aliases p q))
    (union
      [_ p q]
      (let [aliases (u/union aliases p q)
            store (if (u/canonical? aliases q)
                    (rewrite-alias store p q)
                    store)]
        (AliasStore. store aliases))))
#+END_SRC
*** Triple graph (WIP)
...
#+BEGIN_SRC clojure :noweb-ref ctors
  ...
  (defn alias-store
    ...
    (add-bulk (reduce (partial apply u/union) (AliasStore. store (u/disjoint-set)) aliases) triples))
#+END_SRC
