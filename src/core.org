...
For sequential collections defined in a format shown above, we can
compute the cartesian product of each individual pattern to produce a
flat sequence of triples.
...
the following data layout. Unlike with sequences, which allow multiple
subjects, predicates and objects per pattern, for maps we can only
support multiple object values for a given pair of subject/predicate:
