...
#+END_SRC
**** hashEq
#+BEGIN_SRC clojure :noweb-ref tpl-hasheq :noweb yes
  (mix-collection-hash
    <<tpl-hashcode>>
    3)
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref triple
  ...
  ,#+cljs IHash
  ,#+cljs (-hash
          [_] (-> (hash s) (hash-combine (hash p)) (hash-combine (hash o))))
      ...
      (if (and (<= c (count keymap)) (== c (count v) (count (into #{} n))))
        (loop [acc (object-array 3), i 0, n n]
          (if n
            ...
            (Triple. (aget acc 0) (aget acc 1) (aget acc 2))))
        (err/key-error! k))))
...
#+BEGIN_SRC clojure :noweb-ref ctors
  (defn plain-store
    [& triples]
    (add-many (PlainMemoryStore. (hash-map) (hash-map) (hash-map) 0) triples))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref dataset
  (defrecord PlainDataset [models]
    ...
    (remove-many [_ statements]
      (remove-many _ :default statements))
