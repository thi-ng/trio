...
triple store API & query engine for Clojure/ClojureScript. Triple
stores are usually associated with Semantic Web & Linked Data
applications, however encoding graphs as triples is natural too for a
large number of other use cases and this library is aiming to address
these.
...
#+BEGIN_SRC clojure
  ...
  ;; Some characters relations from the 2014 film "The Counselor"
  ;; (contains spoilers!)

  ;; ds will hold a plain triple store
  (def ds
    (add-many
     (plain-store)
  ...
  ;; all subjects
  (subjects ds)
  ;; (jefe green-hornet wireman2 wireman counselor westray councelor laura reiner)
  ...
  (q/select-join-from ds '[[?v killed-by malkina]])
  ;; nil
