...
This library is the result of extracting & refactoring non-RDF related
functionality from the upcoming thi.ng/sema toolkit and make the
underlying graph tools more accessible to a wider audience.
...
to address some of these. Altough currently only an in-memory store is
available, the intention is to have a pluggable storage layer (a bit
like Datomic). Some prototype implementations for Redis, CouchDB &
Cassandra have been in development since last year (but currently
stalled due to other priorities).

The query engine too is far more fully featured than what the example
below's letting on and more demos will be added in due course...
...
The graph above shows some character relationships from the film
"The Counselor", written by Cormac McCarthy & directed by Ridley Scott.
The code below is encoding & analyzing these relationships.
...
#+BEGIN_SRC clojure
  ...
  ;; export the triples as dot file for graphviz
  ;; the file can then be visualized from the command line via:
  ;; `dot -Tpng -o counselor.png counselor.dot`
  (spit "counselor.dot" (q/triples->dot (select ds)))
  ...
  ;; find all triples of characters in love
  (select ds nil 'loves nil)
  ;; ([counselor loves laura] [laura loves counselor] [reiner loves malkina])


  ;; all we know about malkina in object position
  ...
  (q/select-with-bindings ds '[?victim killed-by cartel])
  ...
  (->> '[[?victim [killed-by works-for] ?m]
       ...
       (q/select-join ds)
       (map
        (fn [{:syms [?victim ?lover ?rel ?m ?m2]}]
          (str
           ?victim " was killed on behalf of " ?m ". "
           "the killer's lover, " ?lover ", was killed in turn by " ?m2 ". "
           ?lover " was a " ?rel " " ?victim))))
  ;; ("westray was killed by malkina.
  ;;   the killer's lover, reiner, was killed in turn by cartel.
  ;;   reiner was a friend-of westray")
#+END_SRC
