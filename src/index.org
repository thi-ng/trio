...
[[http://en.wikipedia.org/wiki/Triplestore][triple store]] API and powerful [[http://en.wikipedia.org/wiki/SPARQL][SPARQL]]-like query engine for
...
address some of these. Altough currently only an in-memory store is
...
The [[file:query.org][query engine namespace]] is currently the best source of information
and concrete usage examples. Please take a look!
