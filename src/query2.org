...
expanded cyclically for max-depth hops, e.g:

 : (dfs-forward ds '[?s [mother father] ?o] 3 3)

Else, the fn takes a standard pattern with the subject either a given
constant or qvar. Pattern's object *must* be a qvar:

Select all 'mother' rels from 1 - 3 hops (i.e. from mother -> great-grandmother)
 : (dfs-forward ds '[?s mother ?c] 1 3)

Select only children & grandchildren for given person:

 : (dfs-forward ds '[john father ?c] 2 2)
...
#+BEGIN_SRC clojure :noweb-ref dfs
  (defn subvec-slices
    ...
    (fn [path]
      (map #(subvec path 0 %)
           (range n1 (inc (min (count path) n2))))))
  ...
  (defn dfs-forward*
    ...
    (if (<= (count acc) max)
      (let [acc (conj acc sv)
        ...
        (if (seq r)
          (mapcat #(dfs-forward* ds o p % acc min max)
                  (set (map o (keys r))))
  ...
  (defn dfs-forward
    [ds [s p o] mind maxd]
    (let [o' (make-qvar)
          [?s bmap bfn] (if (qvar? s)
                          [s {} (fn [p] {s (first p) o (peek p)})]
                          (let [?s (make-qvar)]
                            [?s {?s s} (fn [p] {o (peek p)})]))
          mind (max mind 1)
          maxd (max maxd 1)]
      (if (= mind maxd)
        (let [p (if (sequential? p) p [p])
              p (take mind (cycle p))]
          (select-join ds (resolve-prop-path-pattern [s p o])))
        (->> (select-with-bindings ds [?s p o'] bmap)
             (keys)
             (map #(vector (% ?s) (% o')))
             (mapcat #(dfs-forward* ds ?s p (% 1) [(% 0)] mind maxd))
             (mapcat (subvec-slices (inc mind) (inc maxd)))
             (set)
             (map #(vector (bfn %) #{}))
             (into (hash-map))))))
  ...
  (defn dfs-backward*
    ...
    (if (<= (count acc) max)
      (let [acc (conj acc ov)
        ...
        (prn o p ov acc)
        (if (seq r)
          (mapcat #(dfs-backward* ds s p % acc min max)
                  (set (map s (keys r))))
          (if (> (count acc) min) [acc])))
      [acc]))
  ...
  (defn dfs-backward
    [ds [s p o] mind maxd]
    (let [s' (make-qvar)
          [?o bmap bfn] (if (qvar? o)
                          [o {} (fn [p] {s (peek p) o (first p)})]
                          (let [?o (make-qvar)]
                            [?o {?o o} (fn [p] {s (peek p)})]))
          mind (max mind 1)
          maxd (max maxd 1)]
      (if (= mind maxd)
        ...
        (->> (select-with-bindings ds [s' p ?o] bmap)
             (keys)
             (map #(vector (% s') (% ?o)))
             (mapcat #(dfs-backward* ds s' p (% 0) [(% 1)] mind maxd))
             (mapcat (subvec-slices (inc mind) (inc maxd)))
             (set)
             (map #(vector (bfn %) #{}))
             (into (hash-map))))))
...
#+BEGIN_SRC clojure
  ...
  (spit "schmidts.dot" (q/results->dot (q/dfs-forward ds '[?s [mother father] ?c] 2 2)))
#+END_SRC
