...
#+BEGIN_SRC clojure :noweb-ref processor
  ...
  (defmethod compile-query-step :join
    ...
    (fn [res]
      (let [a (qfn res)
            b (select-join from {} {:filter filter :bind bind} where)
  ...
  (defmethod compile-query-step :select
    [qfn {:keys [select from query bind] :as q} _]
    (prn :select select)
    (let [qfn (compile-query-step qfn q :multi-query)
          qfn (compile-query-step qfn q :bind)]
      qfn))
