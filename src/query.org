...
|             |                            | (only mandatory if any subquery doesn't provide its own =:from=)   |            |
|-------------+----------------------------+--------------------------------------------------------------------+------------|
...
Every query spec must contain a =:query= key which is a vector or seq
of subqueries and which themselves are maps. The engine currently
supports the following subquery types:
...
*IMPORTANT:* Each subquery *must* specify one or more search patterns
which are *always* joined in order and are fail fast (i.e. if one
pattern fails the entire subquery fails). The specified set operation
(e.g. optional join) is only applied to merge a subquery's result set
...
subquery: For example, a =:minus= subquery stated as first subquery
