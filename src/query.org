...
pre-processing and optimize code paths for repetive queries.
...
Before discussing the compilation steps dealing with different
toplevel query types (=:select=, =:construct= etc.), let's first take
a look at the various sub-queries and their options:
...
example, a =:minus= subquery stated as first would attempt to subtract
its results from a still empty result set and therefore constitute a
no-op...
...
query options are passed via the options keys listed in the above
table. The =:filter= & =:bind= options both assume a function which is
applied to each unique solution map. However, for most use cases
it is more convenient to only specify these functions for individual
query vars and the following helpers allow us to do so.
...
*Important:* qvar injection via =:bind= is only applied post-filtering.
...
own graph using the =:from= option. If not specified, the subquery
inherits the value stated in the main spec.
...
****** :with (Pre-bound values / restrictions)
...
#+BEGIN_SRC clojure :noweb-ref compiler
  ...
  (defmethod compile-query-step :project-vars
    ...
    (if group
      (compile-query-step qfn q :project-groups)
...
**** Result grouping & aggregates

Query result grouping is only supported for =:select= type queries and
causes the solutions to be returned as map instead of a flat sequence.
The returned map is keyed by the result value of the qvars or function
given for the =:group= keyword in the query spec.
...
grouping criteria, that function be called for each individual result
map and the return value is used to define grouping.
