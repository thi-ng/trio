...
|-------------+---------------------------------+--------------------------------------------------------------------+------------|
| :async      | true                            | If true, execute sub-queries asynchronously in parallel            | N          |
    ...
    {:select    (v/alts
     ...
     :from      #+clj (v/optional
                 (v/alts
                  ...
                  "must satisfy the PModelSelect or PModelConvert protocol"))
     :query     {:* (v/map)}
     :bind      (v/optional
                 (v/alts
                  [(v/map) (v/function)]
...
#+BEGIN_SRC clojure :noweb-ref compiler
  ...
  (defmethod compile-query-step :optional
    ...
    ,#+clj (debug :join-opt optional values)
    (let [opts (query-opts q)]
      (fn [res]
        (let [a (qfn res)]
          (if (or (= ::empty a) (seq a))
            (let [b (select-join from values opts optional)
  ...
  (defmethod compile-query-step :union
    ...
    ,#+clj (debug :union union values)
    (let [opts (query-opts q)]
      (fn [res]
        (let [a (qfn res)
              b (select-join from values opts union)
...
#+BEGIN_SRC clojure :noweb-ref compiler
  (defmethod compile-query-step :query-dispatch
    ...
    ,#+clj (debug :query-dispatch query)
    (loop [qfn qfn, query query]
      (if query
        (let [q (first query)
              from (or (:from q) from)
              q (cond
                 (satisfies? api/PModelSelect from)
                 (assoc q :from from)
...
#+BEGIN_SRC clojure :noweb-ref compiler
  ...
  (defmethod compile-query-step :project-vars-aggregates
    ...
    ,#+clj (debug :aggregates-only avars filter)
  ...
  (defmethod compile-query-step :project-groups
    ...
    ,#+clj (debug :project-groups group aggregate)
...
#+BEGIN_SRC clojure :noweb-ref selectors
  ...
  (defn select-with-bindings-alts
    ...
    (let [vs?      (qvar? s), vp? (qvar? p), vo? (qvar? o)
          ...
          ,#+clj _ #+clj (debug :select-alt qs qp qo)
          res (api/select-with-alts ds qs qp qo)]
      ,#+clj (debug :res-raw res)
      (if (seq res)
