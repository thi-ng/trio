...
just one option (see [[file:core.org][Clojure collection conversion in trio.core
namespace]]).
...
query spec validators. The following helper function is use to apply
  ...
  (defn validate
    ...
    (try
      ...
      (catch #+clj Exception #+cljs js/Error e
        (err/throw! (str "error compiling query: " #+clj (.getMessage e) #+cljs e)))))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref helpers
  ...
  (def qvar?
    ...
    (memoize (fn [x] (and (symbol? x) (re-matches #"^\?.*" (name x))))))
...
an single sub-query based on a heuristic prioritizing patterns with
