...
#+BEGIN_SRC clojure :noweb-ref selectors
  ...
  (defn bind-translator
    [vs? vp? vo? [s p o] [s' p' o']]
    (if vs?
      (if vp? (if vo? {0 s' 1 p' 2 o'} {0 s' 1 p'}) (if vo? {0 s' 2 o'} {0 s'}))
      (if vp? (if vo? {1 p' 2 o'} {1 p'}) (if vo? {2 o'}))))

  (defn translate-unbound-vars
    [bindings vs? vp? vo? [s p o] [s' p' o' :as t]]
  ...
  (defn select-with-prebounds
    ...
    (let [patterns (produce-patterns-with-bound-vars t bindings)
          ...
          vmap (bind-translator vs? vp? vo? t t)
          _ (prn vmap)
          ...
          res-fn (if flt
                   (let [flt (if (map? flt) (compile-filter-fn flt) flt)]
                     #(if (verify %)
                        (let [vbinds (select-renamed-keys {} % vmap)]
                          (if (flt vbinds) vbinds))))
                   #(if (verify %) (select-renamed-keys {} % vmap)))
          res (mapcat
               (fn [p]
                 (let [[qs qp qo] (translate-unbound-vars bindings vs? vp? vo? t p)
                       _ (prn :select p :>> qs qp qo)
                       res (api/select ds qs qp qo)]
                   (if (seq res)
                     (let []
                       (->> res
                            ...
                            (filter identity))))))
               patterns)]
      [(accumulate-result-vars bindings (vals vmap) res) res]))
