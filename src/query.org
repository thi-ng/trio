...
#+BEGIN_SRC clojure :noweb-ref compiler
  ...
  (defn parse-var-bind-spec
    [v spec] (if (map? spec) [(or (:as spec) v) (:fn spec)] [v spec]))
    ...
    (reduce-kv
     (fn [acc v spec]
       (let [[k f] (parse-var-bind-spec v spec)]
         (assoc acc k (if (fn? f) (f (res v)) (res v)))))
     {} svars))
  ...
  (defmethod compile-query-step :project-vars
    ...
    (if group
      ...
      (let [select     (if (sequential? select) select [select])
            svars      (selected-var-keys select identity)
            catch-all? (svars :*)
...
#+BEGIN_SRC clojure
  ...
  ;; retrieve book prices, group by author, compute avg. price & group size per author, filter by avg.
  (q/query
   {:select '[?b ?avg ?n]
    :from ds
    :query '[{:where [[?a author ?b] [?b price ?p]]}]
    :group '?a
    :aggregate {'?p {:as '?avg :fn avg} '?n count}
    :having {'?avg #(> % 10)}})
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref compiler
  ...
  (defmethod compile-query-step :project-aggregates-only
    ...
    (fn [res]
      (->> (qfn res)
           (reduce-kv
            (fn [acc k vals]
              (let [agg (compute-aggregates vars vals)]
           ...
           (reduce-kv
            (fn [acc k vals]
              (let [agg  (compute-aggregates avars vals)
                    vals (if all?
                           (map #(merge % agg) vals)
                           (map #(merge (compute-select-bindings % svars) agg) vals))
  ...
  (defmethod compile-query-step :project-aggregates-simple
    ...
    (fn [res]
      (->> (qfn res)
           (reduce-kv
            (fn [acc k vals]
              (let [vals (map #(compute-select-bindings % vars) vals)
                    vals (if flt (filter flt vals) vals)]
                (if (seq vals)
                  (assoc acc k vals)
                  acc)))
  ...
  (defmethod compile-query-step :project-aggregates
    ...
    (let [select     (if (sequential? select) select [select])
          avars      (selected-var-keys select renamed-vars)
          svars      (validate-selected-vars (selected-var-keys select identity))
          catch-all? (avars :*)
          agg-only?  (every? (set (keys (actual-vars aggregate))) (keys avars))
          filter     (compile-filter-fn having)
          qfn        (fn [res] (group-by group (qfn res)))]
      (prn :svars svars :avars avars)
      (if (seq aggregate)
        (if (and agg-only? (not catch-all?))
          (compile-query-step qfn [aggregate filter] :project-aggregates-only)
