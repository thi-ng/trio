...
#+BEGIN_SRC clojure :noweb-ref compiler
  (defmethod compile-query-step :project-vars
    ...
    (if group
      ...
      (cond
       (= :* select)        qfn
       (sequential? select) (fn [res] (map #(select-keys % select) (qfn res)))
       :else                (compile-query-step
                             qfn (assoc q :select [select]) :project-vars))))
#+END_SRC

**** Result aggregates

#+BEGIN_SRC clojure
  ;; retrieve book prices, group by author, compute avg. price & filter by avg
  (defn avg [vals] (/ (apply + vals) (count vals)))

  (q/query
   {:select {'?p {:as '?avg :fn avg} '?a count}
    :from ds
...
#+BEGIN_SRC clojure :noweb-ref compiler
  (defn compute-aggregates
    ...
    (reduce-kv
     (fn [inner v spec]
       (let [[k f] (if (map? spec) [(:as spec) (:fn spec)] [v spec])]
         (assoc inner k (f (map #(get % v) vals)))))
     {} vars))

  (defmethod compile-query-step :project-aggregates-only
    [qfn [vars filter] _]
    (prn :aggregates-only vars filter)
    (fn [res]
      (->> (qfn res)
           (reduce-kv
            (fn [acc k vals]
              (let [aggregate (compute-aggregates vars vals)]
                (if (filter aggregate)
                  (assoc acc k aggregate)
                  acc)))
            {}))))

  (defmethod compile-query-step :project-aggregates-mixed
    [qfn [avars svars flt] _]
    (prn :mixed-aggregates avars svars flt)
    (fn [res]
      (->> (qfn res)
           (reduce-kv
            (fn [acc k vals]
              (let [agg (compute-aggregates avars vals)
                    vals (map #(merge (select-keys % svars) agg) vals)
                    vals (filter flt vals)]
                (if (seq vals)
                  (assoc acc k (filter flt vals))
                  acc)))
            {}))))
  ...
  (defmethod compile-query-step :project-aggregates-simple
    ...
    (fn [res]
      (->> (qfn res)
           (reduce-kv
            (fn [acc k vals]
              (let [vals (filter flt (mapv #(select-keys % vars) vals))]
                (if (seq vals)
                  (assoc acc k vals)
                  acc)))
            {}))))
  ...
  (defmethod compile-query-step :project-aggregates-all
    [qfn flt _]
    (prn :all-aggregates filter)
    (fn [res]
      (->> (qfn res)
           (reduce-kv
            (fn [acc k vals]
              (let [vals (filter flt vals)]
                (if (seq vals)
                  (assoc acc k (filter vals))
                  acc)))
            {}))))

  (defmethod compile-query-step :project-aggregates
    [qfn {:keys [select group having] :as q} _]
    (prn :group group)
    (let [qvars         (if (sequential? select) select [select])
          [avars svars] (d/bisect map? set qvars)
          avars         (apply merge avars)
          agg?          (not (empty? avars))
          simple?       (not (empty? svars))
          catch-all?    (svars :*)
          filter        (if having (compile-filter-fn having) identity)
          _   (prn :avars avars :svars svars)
          _   (when (and catch-all? (> (count qvars) 1))
                (err/illegal-arg! (str "can't use :* selector with other vars: " qvars)))
          qfn (fn [res] (group-by group (qfn res)))]
      (if agg?
        (if simple?
          (compile-query-step qfn [avars svars filter] :project-aggregates-mixed)
          (compile-query-step qfn [avars filter] :project-aggregates-only))
        (if-not catch-all?
