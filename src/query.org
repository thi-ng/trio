...
variables to be matched), and define a number of set operations how
each individual result set is to be merged to produce the final
solutions.
...
assume a function which is applied to each unique result binding map.
However, for most use cases it is more convenient to only specify these
functions for individual query vars and the following helpers
allow us to do so.
...
in order to cleanly separate logic for each query element and keep the
code legible.
